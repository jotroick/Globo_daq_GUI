				...
				$.ajax({
					...
					success: function (response) {
						...
						$.get('plot_temp_T.xml', function(xml) {
							...
							var series_points=$xml.find('series data point').length;
							for (i = count_b; i < series_points; i++){
								data_input_b.push(eval($($xml.find('series data point')[i]).text()));
								/*console.log("index");
								console.log(i);
								console.log("count");
								console.log(count_a);
						...
						var idt_p = setInterval(function(){
							...
							if(data_input_b.length==0){
								trq_b=setTimeout(requestData_2, 2000); // Time to check if there are more files after everything is plot
								clearInterval(idt_p);  // Stop plotting and start to check for files 
							}
						},800); // Interval between points
					}
				...
				$.ajax({
					...
					success: function (response) {
						...
						$.get('plot_hum_T.xml', function(xml) {
							...
							var series_points=$xml.find('series data point').length;
							for (i = count_c; i < series_points; i++){
								data_input_c.push(eval($($xml.find('series data point')[i]).text()));
								/*console.log("index");
								console.log(i);
								console.log("count");
								console.log(count_a);
						...
						var idt_p = setInterval(function(){
							...
							if(data_input_c.length==0){
								trq_c=setTimeout(requestData_3, 2000); // Time to check if there are more files after everything is plot
								clearInterval(idt_p);  // Stop plotting and start to check for files 
							}
						},800); // Interval between points
					}
...
<!-- ********************************** FOURTH PLOT********************************************************************-->			
		<script>
			var chart_d; // Handle of a plot
			var trq_d;   // Handle of the timer to check if there are more files to plot
			var data_input_d=[]; // Points container
			var count_d=0;
			
			function requestData_4() { 
						
				$.ajax({
					type: "POST",
					url: 'file_plot_speed.php',
					data: {},
					success: function (response) {
						//console.log(response);
						// Get points from the file
						$.get('plot_speed_T.xml', function(xml) {
							var $xml = $(xml);
							var series = chart_d.series[0];
							
							var series_points=$xml.find('series data point').length;
							for (i = count_d; i < series_points; i++){
								data_input_d.push(eval($($xml.find('series data point')[i]).text()));
								/*console.log("index");
								console.log(i);
								console.log("count");
								console.log(count_a);
						...
						// Plot points
						var idt_p = setInterval(function(){
							if(data_input_d.length>0){
								clearInterval(trq_d); // Stop checking for files and start to plot
								var po2=data_input_d.shift(0,1); // Take first element from the vector
								//console.log(chart_a.series[0]);
								chart_d.series[0].addPoint(po2,true,chart_d.series[0].activePointCount>=19); // Plot element
							}
							if(data_input_d.length==0){
								trq_d=setTimeout(requestData_4, 2000); // Time to check if there are more files after everything is plot
								clearInterval(idt_p);  // Stop plotting and start to check for files 
							}
						},800); // Interval between points
					}
...
<!-- ********************************** FIFTH PLOT********************************************************************-->	
		<script>
			var chart_e; // Handle of a plot
			var trq_e;   // Handle of the timer to check if there are more files to plot
			var data_input_e=[]; // Points container
			var count_e=0;
			
			function requestData_5() { 
						
				$.ajax({
					type: "POST",
					url: 'file_plot_altitud.php',
					data: {},
					success: function (response) {
						//console.log(response);
						// Get points from the file
						$.get('plot_alt_T.xml', function(xml) {
							var $xml = $(xml);
							var series = chart_e.series[0];
							
							var series_points=$xml.find('series data point').length;
							for (i = count_e; i < series_points; i++){
								data_input_e.push(eval($($xml.find('series data point')[i]).text()));
								/*console.log("index");
								console.log(i);
								console.log("count");
								console.log(count_a);
						...
						// Plot points
						var idt_p = setInterval(function(){
							if(data_input_e.length>0){
								clearInterval(trq_e); // Stop checking for files and start to plot
								var po2=data_input_e.shift(0,1); // Take first element from the vector
								//console.log(chart_a.series[0]);
								chart_e.series[0].addPoint(po2,true,chart_e.series[0].activePointCount>=19); // Plot element
							}
							if(data_input_e.length==0){
								trq_e=setTimeout(requestData_5, 2000); // Time to check if there are more files after everything is plot
								clearInterval(idt_p);  // Stop plotting and start to check for files 
							}
						},800); // Interval between points
					}
